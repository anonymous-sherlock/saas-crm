...
export async function CreateLead(req: NextRequest, res: NextResponse) {
    try {
        ...
        const { campaignCode, data: { name, phone, address } } = parsedBody
        ...
        if (err instanceof ZodError) {
            return NextResponse.json({
                success: false, error: "error", message: formatZodError(err) ?? "Validation error occurred"
            } satisfies ApiLeadUploadError, { status: 400 });
        }
