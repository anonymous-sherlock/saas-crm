...
async function UserLeadsPage({ params: { userId }, searchParams: { date } }: UserLeadsPageProps) {
  const user = await db.user.findFirst({ where: { id: userId } });
  if (!user) notFound();
  const { from, to } = getDateFromParams(date);
  const leads = await lead.getAllLeads({ userId: user.id, date: { from, to } });
  return (
    <>
          ...
          <div>
            <div className="flex space-x-4">
              <PageHeaderHeading size="sm" className="flex-1 capitalize">
                {user.name}&apos;s <span className="text-green-600">All Leads</span>
              </PageHeaderHeading>
            </div>
