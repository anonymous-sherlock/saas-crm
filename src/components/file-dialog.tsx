  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        {triggerStyle === "Button" ?
          <Button variant="outline" disabled={disabled} className="h-11">
            {labelText}
            <span className="sr-only">{labelText}</span>
          </Button>
          : <div className={cn('border h-64 border-dashed border-gray-300 rounded-lg', isDragActive && "border-muted-foreground/50 bg-fuchsia-100",)}>
      ...
      <DialogContent className="sm:max-w-[480px] ">
        ...
        <div
          {...getRootProps()}
          className={cn(
            "group relative mt-8 grid h-48 w-full cursor-pointer place-items-center rounded-lg border border-dashed border-muted-foreground/25 px-5 py-2.5 text-center transition hover:bg-muted/25",
