...
import { deleteProducts, upsertProduct } from "@/lib/actions/product.action";
...
type ProductFormProps = {
  product?: GetSingleProductType;
  type: "create" | "update";
  userId: string;
};

export function ProductForm({ type, product, userId }: ProductFormProps) {
  ...
  const { data: fileData, fetchNextPage, isFetchingNextPage, isLoading } = trpc.media.getUserMedia.useInfiniteQuery({ limit: 12, userId: userId }, { getNextPageParam: (lastPage) => lastPage.nextCursor });
  ...
  function handleProductDelete() {
    startDeletingTransition(async () => {
      ...
      if (type === "update") {
        ...
        try {
          deleteProducts({ productIds: [product.id], userId: userId }).then((data) => {
            setError(data?.error);
            setSuccess(data?.success);
            if (data?.success) {
              setCloseModal();
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} method="post" className="flex flex-col lg:grid lg:grid-cols-5 items-start gap-6 space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <NextInput type="text" aria-label="Product Name" size="sm" placeholder="Nutra Bay" variant="faded" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <NextInput type="text" aria-label="Product Name" size="sm" placeholder="1,999" variant="faded" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
                    ...
                    <Popover isOpen={open} onOpenChange={setOpen} placement="bottom">
                      <PopoverTrigger className="aria-expanded:scale-[1]">
                        <FormControl>
                          ...
                          >
                            {field.value ? PRODUCT_CATEGORIES.find((cat) => field.value === cat.value)?.label : "Select a Category"}
                            <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                          </Button>
                        </FormControl>
                      </PopoverTrigger>
                      <PopoverContent className="min-w-[275px] p-0 gap-0 divide-y divide-default-300/50 dark:divide-default-100/80 bg-content1 overflow-hidden shadow-small rounded-medium">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <NextInput type="text" aria-label="Product Name" size="sm" placeholder="-1 for unlimited stocks" variant="faded" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
                ...
                <FormItem className={cn(index !== 0 && "-mt-6")}>
                  <div className="flex">
                    <div className="flex-1">
                      <FormLabel className={cn(index !== 0 && "sr-only")}>Media URLs</FormLabel>
                      <FormDescription className={cn(index !== 0 && "sr-only")}>Add links of your Video, youtube, vimeo or any other source.</FormDescription>
                    </div>
                    {index === 0 ? (
                      <Button type="button" variant="secondary" size="sm" className="mt-2 md:w-1/4 border" onClick={() => append({ value: "" })}>
                        Add URL
                      </Button>
                    ) : null}
                  </div>
        ...
        {type === "update" ? (
          <Button type="button" variant="destructive" className={cn("w-full col-span-1 !mt-0")} disabled={isDisabled} onClick={handleProductDelete}>
            {isDeleting ? (
              <>
                <Spinner />
                Deleting...
        ...
        <Button type="submit" disabled={isFormUnchanged || isPending} className={cn("w-full !mt-0")}>
          {isPending ? (
            <React.Fragment>
              {type === "create" ? (
                <>
