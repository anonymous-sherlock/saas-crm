...
interface AllUserMediaLibraryContentProps {
  selectedFile: Media[];
  setSelectedFile: React.Dispatch<React.SetStateAction<Media[]>>;
  maxFiles: number;
}

const AllUserMediaLibraryContent: FC<AllUserMediaLibraryContentProps> = ({ selectedFile, setSelectedFile, maxFiles }: AllUserMediaLibraryContentProps) => {
  const {
    data: fileData,
    fetchNextPage,
    isFetchingNextPage,
    isLoading,
    isInitialLoading,
  } = trpc.media.getAllMedia.useInfiniteQuery({ limit: 12 }, { getNextPageParam: (lastPage) => lastPage.nextCursor });
  const data = fileData ? fileData.pages.flatMap((data) => data.mediafiles) : [];
  const lastImageRef = useRef<HTMLDivElement>(null);

  const isFetching = isInitialLoading || isLoading || isFetchingNextPage;
...
export default AllUserMediaLibraryContent;
