...
import AllUserMediaLibraryContent from "./all-user-media-library-tab-content";
import { useSession } from "next-auth/react";
import { allowedAdminRoles } from "@/lib/auth.permission";

type TabType = "uploader" | "library" | "all_user_media";
...
export const MediaDialog = ({
  ...
  const tabContents: Record<TabType, JSX.Element> = {
    ...
    library: (
      <MediaLibraryTabContent data={images} fetchNextPage={fetchNextPage} isFetchingNextPage={isFetchingNextPage} selectedFile={selectedFile} setSelectedFile={setSelectedFile} maxFiles={maxFiles} />
    ),
    all_user_media: <AllUserMediaLibraryContent maxFiles={maxFiles} selectedFile={selectedFile} setSelectedFile={setSelectedFile} />,
  };
  ...
  return (
    <>
      {children ? (
        <div onClick={openModal} className="h-full w-full">
          {children}
        </div>
      ) : (
        <>
                ...
                <label htmlFor="" className="flex flex-col items-center justify-center w-full h-full rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="mb-2 text-sm text-zinc-700">
                      <span className="font-semibold">Click to select</span> media file
                    </p>
                    <p className="text-xs text-zinc-500">Image max (up to {maxFiles})</p>
                  </div>
                </label>
              </div>
            </div>
              ...
              {selectedFile.map((file, i) => (
                <Fragment key={file.id}>
                  <div className="relative group">
                    <Zoom key={i}>
                      <Image src={file.url} alt={file.name} className="h-20 w-20 shrink-0 rounded-md object-cover object-center" width={80} height={80} />
                    </Zoom>
                    <span
                      className="absolute cursor-pointer -top-2 -right-2 z-[999] size-5 flex justify-center items-center rounded-full bg-red-700 text-white opacity-0 pointer-events-none transition-opacity duration-300 group-hover:opacity-100 group-hover:pointer-events-auto"
        ...
        <ModalContent>
          {(onClose) => (
            <>
              ...
              <ModalBody>{tabContents[selectedTab]}</ModalBody>
              <ModalFooter>
                {(selectedTab === "library" || selectedTab === "all_user_media") && (
                  <Button color="primary" disabled={selectedFile.length === 0} onClick={onOpenChange}>
                    Select
                  </Button>
                )}
              </ModalFooter>
