...
export function OnboardingForm({ className, userId }: OnboardingFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof CompanyDetailsSchema>) {
    startTransition(async () => {
      ...
      setSuccess("");
      try {
        await addCompanyDetails({data:values,userId}).then(async (data) => {
          if (data?.error) {
            setError(data.error);
  ...
  return (
    <Card className={cn("p-6 w-full max-w-3xl bg-white mx-auto", className)}>
          ...
          <ul className="flex relative justify-between">
            {steps.map((step, index) => (
              <li key={step} className="flex relative z-20 text-center text-xs flex-col items-center gap-4 text-[#536387] md:text-[16px] font-semibold">
                <span className={cn("w-10 h-10 bg-[#DDE3EC] rounded-full shrink-0 flex justify-center text-xl items-center", index <= currentStep && "active")}>{index + 1}</span>
                {step}
              </li>
            ))}
            <span
              className={cn("absolute left-0 right-0 top-[20%] h-2 w-[80%] md:w-[88%] mx-auto z-0 bg-[#DDE3EC]")}
              style={{ background: gradientColor, transition: "background 0.7s ease" }}
            ></span>
          </ul>
        </div>
      </CardHeader>
      <CardContent className="mt-2 w-full p-0">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} method="post" className="flex flex-col items-start space-y-4">
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-card-foreground text-sm">
                            Company Name <span className="text-muted-foreground italic">(Type N/A if you are individual).</span>
                          </FormLabel>
                          <FormControl>
                            <Input
                              type="text"
                              ...
                              startContent={<Building className="text-xl w-5 h-5  text-default-400 pointer-events-none flex-shrink-0" />}
                              {...field}
                            />
                          </FormControl>
                          <FormMessage />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="col-span-2">
                          <FormLabel className="text-card-foreground text-sm">
                            GST Number <span className="text-muted-foreground italic">(Type N/A if you are individual).</span>
                          </FormLabel>
                          <FormControl>
                            <Input type="text" size="sm" variant="faded" placeholder="GST Number" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input type="text" size="sm" variant="faded" placeholder="Zipcode" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="col-span-2">
                          ...
                          <FormControl>
                            <Input type="text" size="sm" variant="faded" placeholder="Landmark" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="col-span-2">
                          ...
                          <FormControl>
                            <Input type="text" size="sm" variant="faded" placeholder="Address" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
            ...
            {currentStep < STEPS_AMOUNT ? (
              <div className="flex w-full gap-2 mt-4 justify-end">
                {currentStep > 0 && (
                  <Button type="button" className={cn("ml-auto shrink-0")} disabled={isPending || currentStep === 0} variant="secondary" onClick={handleGoBackToPreviousStep}>
                    Back
                  </Button>
                )}
                <Button
              ...
              <div className="flex w-full gap-2 mt-4">
                <Button type="button" className={cn("ml-auto shrink-0")} disabled={isPending} variant="secondary" onClick={handleGoBackToPreviousStep}>
                  Back
                </Button>

                <Button type="submit" autoSave="false" className={cn("shrink-0")} disabled={isPending || currentStep < STEPS_AMOUNT || !form.formState.isValid}>
