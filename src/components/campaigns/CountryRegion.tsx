...
const CountryRegion = () => {
  ...
  const { control, setValue, clearErrors, formState, setError } = useFormContext();

  const selectedRegionRef = useRef<Array<string>>([]);

  const handleRegionSelect = (region: string) => {
    selectedRegionRef.current = selectedRegionRef.current.includes(region) ? selectedRegionRef.current.filter((r) => r !== region) : [...selectedRegionRef.current, region];
          ...
          <FormItem>
            ...
            <FormControl>
              <Popover open={regionOpen} onOpenChange={setRegionOpen}>
                <PopoverTrigger asChild>
                  <FormControl>
                    <Button variant="outline" role="combobox" aria-expanded={regionOpen} className={cn("w-full justify-between h-11", !field.value && "text-muted-foreground")}>
                        ...
                        {allCountries
                          ...
                          .map((country) =>
                            country[2].map((region, index) => (
                              ...
                              >
                                <Check className={cn("mr-1 h-4 w-4", selectedRegionRef.current.includes(region[0]) ? "opacity-100" : "opacity-0")} />
                                <span>{region[0]}</span>
